...
foreach($listadoPago as $listaa) {
    ...
    $sentencia=$pdo->prepare("UPDATE ctactes SET ver=now() WHERE IDPAGO='$aaa'  ");
    $sentencia->execute();   
}
...
foreach($listadoVenta as $listab) {
    ...
    $sentencia=$pdo->prepare("UPDATE ctactes SET ver='$idCierre' WHERE IDVENTA='$bbb'  ");
    $sentencia->execute();   
}
...
$sentencia=$pdo->prepare("UPDATE tbldetallpagos SET DESCARGADO='$idCierre'*-1 WHERE DESCARGADO='$idCierre'  ");
$sentencia->execute(); 

$sentencia=$pdo->prepare("UPDATE tbldetalleventa SET DESCARGADO='$idCierre'*-1 WHERE DESCARGADO='$idCierre'  ");
$sentencia->execute(); 

$sentencia=$pdo->prepare("UPDATE cierrescaja SET Numero='$idCierre'*-1 WHERE Numero='$idCierre'  ");
$sentencia->execute(); 
...
$mensaje = 'Esta caja ha sido borrada'; 
...
$mail->Subject = EMPRESA." CAJA BORRADA del ".'N° '.$articulo; // Este es el titulo del email.
