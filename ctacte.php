...
<?php foreach($listaProductos as $producto) { ?>
    ...
    <?php } ?>    
              ...
              <div class="col-12">
              <form action="ctacte.php" method="get" class="form-sample">
...
//print_r($listaProductos);
   ...
   $sentencia=$pdo->prepare("SELECT * FROM ctactes where SOCIO='$filtro' and Observaciones='Pesos' and  (ctactes.FechaRecibo BETWEEN date('$desde') AND date('$hasta')) and ver is NULL order by FechaRecibo asc , TipoPago desc");
   $sentencia->execute();
   $listaProductos=$sentencia->fetchAll(PDO::FETCH_ASSOC);
   //print_r($listaProductos);
...
if ( $cta['CERRADO']>-1) { ?><span>
            ...
            <div style="align:right;"><a href="excelCuentaCliente.php?desde=<?php echo $desde?>&hasta=<?php echo $hasta?>&filtro=<?php echo $filtro?>&SaldoComienzo=<?php echo $SaldoComienzo?>"><button type="button" class="btn btn-secondary">Exportar a Excel</button></a></div> 
