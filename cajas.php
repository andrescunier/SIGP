...
?>
    ...
    <?php }?>
     <div >
     <?php
            $sentencia=$pdo->prepare('SELECT SUM(MovCaja) as mtotal FROM cajas');
            $sentencia->execute();
         ...
         ?>
<div><h1>Movimiento Total de Cajas  </h1></div>
<div >
<h3 class="text-right"> Saldo Actual $ <?php print ($totales['mtotal'])?> </h3>
           ...
           <?php
            $sentencia=$pdo->prepare('SELECT * FROM cajas  where `MovCaja`!=0 order by FechaRecibo asc');
            $sentencia->execute();
