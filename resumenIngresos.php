...
<?php } ?> </h1>
    ...
    <?php
            $sentencia=$pdo->prepare("SELECT categorias.categoria, SUM( tbldetallpagos.CANTIDAD * tbldetallpagos.PRECIOUNITARIO) AS TOTAL FROM tblctasctes LEFT JOIN tbldetallpagos on tblctasctes.ID=tbldetallpagos.IDVENTA LEFT JOIN MediosDePago ON tbldetallpagos.IDPRODUCTO=MediosDePago.ID  left join clientes ON tblctasctes.SOCIO=clientes.SOCIO left join  categorias ON clientes.DOCUMENTO=categorias.ID LEFT JOIN ctactes ON tblctasctes.ID=ctactes.IDPAGO  where  tblctasctes.status='Cobro' AND tbldetallpagos.DESCARGADO>-1 AND (tblctasctes.Fecha BETWEEN date('$desde') AND date('$hasta')) GROUP BY categorias.categoria ORDER BY TOTAL DESC;");
            $sentencia->execute();
...
<?php foreach($listaProductos as $producto) { ?>
  ...
  <td width="10%" text-center ><?php $order=$order+1; echo ($order) ?></td> 
                     ...
                     <td width="20%"><?php echo $producto['categoria'] ?></td>
                     
                     <td width="15%">$<?php echo number_format($producto['TOTAL'], 2, ',', ' ') ?></td>
            ...
            <div style="align:right;"><a href="excelListadoIngresos.php?desde=<?php echo $desde?>&hasta=<?php echo $hasta?>&filtro=<?php echo $filtro?>&SaldoComienzo=<?php echo $SaldoComienzo?>"><button type="button" class="btn btn-secondary">Exportar a Excel</button></a></div> 
