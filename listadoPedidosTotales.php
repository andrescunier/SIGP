...
?>
     ...
     <tbody>
                     <tr>
                        ...
                        <th width="10%" text-center>Estado Actual</th>
                        <th width="15%" text-center>Cambio de Estado</th>
         ...
         //   $listaProductos=$sentencia->fetchAll(PDO::FETCH_ASSOC);
            ...
            $total=0;
               $sentencia=$pdo->prepare('SELECT tblventas.ID , tblventas.Fecha ,tblventas.domicilio, tblventas.Correo , tblventas.SOCIO , clientes.APELLIDO, clientes.NOMBRE, tblventas.PayPalDatos, tblventas.horario from tblventas INNER JOIN clientes on tblventas.SOCIO=clientes.SOCIO WHERE tblventas.status="Vendido"  ORDER BY tblventas.ID desc');
               $sentencia->execute();
               $listaProductos=$sentencia->fetchAll(PDO::FETCH_ASSOC);
         ...
         <?php foreach($listaProductos as $producto) { ?>
            ...
            <table class="table table-striped">
                <tbody>
                     ...
                     <td width="15%" text-center ><?php echo $producto['horario'] ?></td>
