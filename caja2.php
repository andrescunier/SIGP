...
?>
    ...
    <?php }?>
     <div >
     <?php
            $sentencia=$pdo->prepare('SELECT SUM(MovCaja) as mtotal FROM cajas where (TipoPago="caja" and Observaciones="Pesos")');
            $sentencia->execute();
         ...
         ?>
<div><h1>Movimiento de Caja Pesos  </h1></div>
<div >
<h3 class="text-right"> Saldo Actual $ <?php print ($totales['mtotal'])?> </h3>
           ...
           <?php
            $sentencia=$pdo->prepare('SELECT * FROM cajas where (TipoPago="caja" and Observaciones="Pesos") order by FechaRecibo asc');
            $sentencia->execute();
