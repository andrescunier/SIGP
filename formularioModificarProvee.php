...
<?php foreach($listaProductos as $producto) { ?>
    ...
    <?php } ?>  
         ...
         <?php foreach($listaProductos as $producto) { ?>
                  ...
                  <form class="cmxform" id="commentForm"  action="actualizaProvee.php" method="post">
                  ...
                  <input type="hidden" class="form-control" name="SOCIO" placeholder="Nombre" value="<?php echo ( $producto['SOCIO']);?>">
                      ...
                      <?php              $sentencia=$pdo->prepare("select * from cuentas where ID='$cuntina'");
         $sentencia->execute();
         $planCuentas=$sentencia->fetchAll(PDO::FETCH_ASSOC);
         foreach ($planCuentas as $cuentas){ ?> 
            
<option  selected value="<?php echo $cuentas['ID'];?>"><?php echo $cuentas['Codigo'].' / '.$cuentas['Cuenta'].' / '.$cuentas['subCuenta'];?></option>
<?php }          ?> 
...
<?php }          ?> 
            ...
            <input type="hidden" class="form-control" name="Correo" placeholder="Ingresar Categoria"  value="<?php echo ( $producto['Correo electronico']);?>">
                      <div class="form-group">
