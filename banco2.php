...
?>
    ...
    <?php }?>
     <div >
     <?php
            $sentencia=$pdo->prepare('SELECT SUM(MovCaja) as mtotal FROM cajas where (TipoPago="banco" and Observaciones="Dolar")');
            $sentencia->execute();
         ...
         ?>
<div><h1>Movimiento de Banco Dolar  </h1></div>
<div >
<h3 class="text-right"> Saldo Actual USD $  <?php print ($totales['mtotal'])?> </h3>
</div>
 


           <?php
            $sentencia=$pdo->prepare('SELECT * FROM cajas where (TipoPago="banco" and Observaciones="Dolar") order by FechaRecibo asc');
            $sentencia->execute();
         ...
         <?php foreach($listaProductos as $producto) { ?>
            ...
            <table class="table table-striped">
                ...
                <tbody>
                        ...
                        <td width="15%" text-right>USD $<?php echo $producto['MovCaja'] ?></td>
                        
                        <?php $saldo+=$producto['MovCaja'] ?>
                        <td width="20%" text-right>USD $<?php echo $saldo ?></td>
