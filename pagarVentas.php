...
<?php 
    ...
    if($_POST){
        ...
        $sentencia=$pdo->prepare("INSERT INTO `tblventas` (`ID`, `ClaveTransaccion`, `PayPalDatos`, `Fecha`, `SOCIO`, `Total`, `status`,`domicilio`,`Complejo`) 
        VALUES (NULL, :ClaveTransaccion, '',:Fecha,:SOCIO, :Total, 'Vendido',:domicilio,:Complejo);");
        ...
        $sentencia->execute(); 
            ...
            foreach ($_SESSION['CARRITOVENTA'] as $indice => $producto){
                ...
                VALUES (NULL,:IDVENTA, :IDPRODUCTO, :PRECIOUNITARIO, :CANTIDAD, '0',:Complejo);");
            ...
            VALUES (now(), :Fecha, :TipoC,:Comprobante,:SOCIO,:Observaciones,'Venta',:MovCtaCte,:Complejo,:IDVENTA);");    
            $sentencia->bindParam(":Fecha",$Correo);
...
window.setTimeout(function() {
    window.location.replace('listadoClientesVentas.php');
}, 3000);
