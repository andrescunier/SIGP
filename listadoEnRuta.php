...
?>
     ...
     <tbody>
         ...
         //   $listaProductos=$sentencia->fetchAll(PDO::FETCH_ASSOC);
            ...
            $total=0;
               $sentencia=$pdo->prepare('SELECT tblventas.ID , tblventas.Fecha ,tblventas.domicilio, tblventas.Correo , tblventas.SOCIO , clientes.APELLIDO, clientes.NOMBRE, tblventas.PayPalDatos from tblventas INNER JOIN clientes on tblventas.SOCIO=clientes.SOCIO WHERE tblventas.status="Vendido" AND tblventas.PayPalDatos="Ruteado" ORDER BY tblventas.ID desc');
               $sentencia->execute();
               $listaProductos=$sentencia->fetchAll(PDO::FETCH_ASSOC);
                ...
                <tbody>
                     ...
                     <!--   <td width="10%" text-center ><?php echo $producto['PayPalDatos'] ?></td> -->
                        <td scope="row" width="10%">
                            <form action="actualizaRuteaPedido.php" method="post">
                             <input type="hidden"
                            name="ID" 
                            id="ID" 
                            ...
                            id="RUTEO" 
                            value="Entregado">   
                            <button class="btn btn-danger"  type="submit" name="btnAccion" value="Entregado">Entregado</button> 
                            </form>
